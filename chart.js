...
if (typeof(channelKeys) == 'undefined') {
	var channelKeys = [];

	// Channel 1)
	/*
	channelKeys.push({channelNumber:12345, name:'Give me a name',key:'set a key',
	   fieldList:[{field:1,axis:'T'},{field:2,axis:'H'}]});
	*/
	
	// Channel 2)
	channelKeys.push({channelNumber:27915, name:'Temp & Hum (Outdoor)',key:'3KOEYAXFLFUPNPIT',
	   fieldList:[{field:1,axis:'T'},{field:2,axis:'H'}]});
}

if (typeof(thingspeak_median) == 'undefined') {
	var thingspeak_median = '';
}
...
$(document).ready(function() {
	...
	function loadThingSpeakChannel(sentChannelIndex, channelNumber, key, sentFieldList) {
		...
		$.getJSON('https://www.thingspeak.com/channels/' + channelNumber + '/feed.json?callback=?&amp;offset=0&amp;median='+thingspeak_median+'&amp;results=2500;key=' + key, function(data) {
			// if no access
			if (data == '-1') {
			...
			for (var fieldIndex = 0; fieldIndex < fieldList.length; fieldIndex++) // iterate through each field
			{
				...
				for (var h = 0; h < data.feeds.length; h++) // iterate through each feed (data point)
				{
					...
					var isValidPoint = !isNaN(parseInt(v)); // && parseInt(v) > 0;
					if (isValidPoint) {
						fieldList[fieldIndex].data.push(p);
					}
				}
				fieldList[fieldIndex].name = eval("data.channel.field" + fieldList[fieldIndex].field); //+'-'+channelIndex;
			}
	...
	function createChart() {
		...
		var chartOptions = {
								...
								if (document.getElementById("Update").checked) {
									for (var channelIndex = 0; channelIndex < channelKeys.length; channelIndex++) // iterate through each channel
									{
										(function(channelIndex) {
											...
											$.getJSON('https://www.thingspeak.com/channels/' + channelKeys[channelIndex].channelNumber + '/feed/last.json?callback=?&amp;offset=0&amp;median='+thingspeak_median+'&amp;location=false;key=' + channelKeys[channelIndex].key, function(data) {
			...
			rangeSelector: {
				...
				inputEnabled: true,
				selected: 2 //Change to 3rd button as default (index starts at 0)
			},
			title: {
				text: ''
			},
...
function loadChannelHistory(sentChannelIndex, channelNumber, key, sentFieldList, sentNumLoads, maxLoads) {
	...
	// get the Channel data with a webservice call
	$.getJSON('https://www.thingspeak.com/channels/' + channelNumber + '/feed.json?callback=?&amp;offset=0&amp;median='+thingspeak_median+'&amp;start=2013-01-20T00:00:00;end=' + end + ';key=' + key, function(data) {
		// if no access
		if (data == '-1') {
			$('#chart-container').append('This channel is not public.  To embed charts, the channel must be public or a read key must be specified.');
			window.console && console.log('Thingspeak Data Loading Error');
